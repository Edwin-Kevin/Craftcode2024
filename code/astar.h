...
{
    if(x >= 0 && x < n && y >= 0 && y < n && (ch[x][y] == '*' || ch[x][y] == 'A'))
    {
        return true;
    }
    return false;
...
/*
    ...
    @ret: none
*/
void aStarSearch(int srcX, int srcY, int destX, int destY)
{
    // 若起点或终点不可达
    if(!isUnBlocked(srcX, srcY) || !isUnBlocked(destX, destY))
    {
        return;
    }
    ...
    while(!openList.empty())
    {
        ...
        for(int k = 0; k < 4; ++k)
        {
            ...
            if(!isUnBlocked(newX, newY) && !closedList[newX][newY])
            {
