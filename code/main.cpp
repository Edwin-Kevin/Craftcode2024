...
std::multimap<int, int> weightToIndex;
...
void Init()
{
    ...
    for(int i = 0; i < berth_num; i ++)
    {
        ...
        // 将权重存入集合
        weightToIndex.insert(std::make_pair(berth[i].weight, i));
        minWeights.insert(berth[i].weight);
...
int main()
{
    ...
    for(int frame = 1; frame <= 15000; ++ frame)
    {
        ...
        for (int weight : minWeights) {
            auto range = weightToIndex.equal_range(weight);
            for (auto it = range.first; it != range.second; ++it) {
                int berthIndex = it -> second;
                if(boat[cnt].status == BOAT_STATUS_NORMAL && boat[cnt].pos == -1)
                    printf("ship %d %d\n", cnt, berthIndex);
                // std::cout << "Weight: " << it->first << ", Berth Index: " << it->second << std::endl;
            }
        }
