...
int second_nearest_berth(int availmap[n][n], int x, int y)
{
    ...
    {
        if(((abs(berth[i].x - x) + abs(berth[i].y - y)) > min) &&
            ((abs(berth[i].x - x) + abs(berth[i].y - y)) < sec_min)
                && availmap[berth[i].x][berth[i].y] == availmap[x][y])
        {
...
int nearest_berth_after_11000(int availmap[n][n], int x, int y)
{
    ...
    for(int i = 0; i < berth_num; i++)
    {
        if(((abs(berth[i].x - x) + abs(berth[i].y - y)) < min) && berth[i].selected
           && availmap[berth[i].x][berth[i].y] == availmap[x][y])
        {
            min = abs(berth[i].x - x) + abs(berth[i].y - y);
            nearestid = i;
...
int second_nearest_berth_after_11000(int availmap[n][n], int x, int y)
{
    ...
    {
        if(((abs(berth[i].x - x) + abs(berth[i].y - y)) > min) &&
           ((abs(berth[i].x - x) + abs(berth[i].y - y)) < sec_min) && berth[i].selected
           && availmap[berth[i].x][berth[i].y] == availmap[x][y])
        {
            sec_min = abs(berth[i].x - x) + abs(berth[i].y - y);
            second_nearestid = i;
